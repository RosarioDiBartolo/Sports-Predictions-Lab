...
def _team_snapshot(history: deque[dict[str, object]]) -> dict[str, float]:
    ...
    for player in players:
        starts = [
            match for match in history if player in match["starters"]  # type: ignore[operator]
        ]
    ...
    return {
        "player_expected_strength": _weighted_average(
            starter_probability, strength
        ),
        ...
        "player_bench_quality": _weighted_average(
            bench_probability, strength, limit=5
        ),
