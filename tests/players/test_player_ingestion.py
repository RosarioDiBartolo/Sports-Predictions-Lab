...
from football_odds.players.coverage import ApiFootballClient
from football_odds.players.observations import (
...
def test_ingestion_is_idempotent_and_keeps_starters_and_bench(tmp_path):
    ...
    with database.connect() as connection:
        ...
        assert (
            connection.execute("SELECT COUNT(*) FROM lineup_players").fetchone()[0]
            == 26
        )
        roles = dict(
            connection.execute(
